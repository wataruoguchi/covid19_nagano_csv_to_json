...
import { setLabelFromDateStr, addDate1 } from "../../converter/utils";

function buildDataByPatientAndTestCount(
  patientRows: patient[],
  ...
  if (
    ...
    const reportDateStr = setLabelFromDateStr(
      patientRows[patientRows.length - 1].pubYMD,
      "",
      addDate1
    )("reportDate");
...
function buildDataByTestCount(testCountRows: testCount[]) {
  if (testCountRows && testCountRows.length) {
    const reportDateStr = setLabelFromDateStr(
      testCountRows[testCountRows.length - 1].YMD,
      "",
      addDate1
    )("reportDate");
...
function buildDataByCallCenter(callCenterRows: callCenter[]) {
  return callCenterRows && callCenterRows.length
    ? {
        contacts: {
          date: setLabelFromDateStr(
            callCenterRows[callCenterRows.length - 1].YMD,
            "",
            addDate1
          )("reportDate"),
          data: callCenterRows
